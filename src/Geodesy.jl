...
function _f2c(x::Real, r::Real, level::Integer)
    t = mod(x, r)
...
function areacode(lat::Real, lon::Real)
    (slat, rlat) = _f2c(90.0 - lat, 180.0, 2)
    (slon, rlon) = _f2c(180.0 + lon, 360.0, 2)
    clat = string(round(Int, rlat / 0.01))
    clon = string(round(Int, rlon / 0.01))
    return slat * clat * slon * clon
end

function _s2f(s::String, dr::Real)
    f3 = Int(s[3] - '0') * 0.01
...
function areadecode(s::String)
    theta = _s2f(String(s[1:3]), 0.05)
    phi = _s2f(String(s[4:6]), 0.1)
    return (round(90.0 - theta; digits = 2), round(phi - 180.0; digits = 2))
end
