...
struct WaveFrame <: Frame
    ...
    data::AbstractVector
    function WaveFrame(n="", s="", dv="", cmp="", bt=DateTime(2021), dt=0.01, data=[], npts=0,
                       meta=Dict{String,Any}(); id="", fill_meta=false)
        if isempty(id)
            id = join([n, s, dv, cmp], '.')
        end
        if npts == 0
...
sacheadtrans = (int2other=Dict([("iftype", iftype), ("idep", idep), ("iztype", iztype),
                                ("ievtyp", ievtyp), ("iqual", iqual), ("isynth", isynth),
                                ("imagtyp", imagtyp), ("imagsrc", imagsrc)]),
                other2int=Dict([("iftype", iftyper), ("idep", idepr), ("iztype", iztyper),
                                ("ievtyp", ievtypr), ("iqual", iqualr), ("isynth", isynthr),
                                ("imagtyp", imagtypr), ("imagsrc", imagsrcr)]))
